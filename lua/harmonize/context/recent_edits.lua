-- Recently edited regions across the project. Each buffer is attached with
-- on_lines; every edit is tracked with an extmark so later line insertions
-- and deletions move the region with the text. snapshot() reads the extmark
-- positions and extracts whole lines around each region.
local api = vim.api

local M = {}

M.namespace = api.nvim_create_namespace 'harmonize-recent-edits'

-- Entries across all tracked buffers: { bufnr, extmark_id, timestamp }.
...
local function entry_region(entry)
    local ok, pos = pcall(api.nvim_buf_get_extmark_by_id, entry.bufnr, M.namespace, entry.extmark_id, {
        details = true,
    })
...
local function prune()
    local max_edits = require('harmonize').config.context_sources.recent_edits.max_edits
    while #entries > max_edits do
        local oldest = table.remove(entries, 1)
        pcall(api.nvim_buf_del_extmark, oldest.bufnr, M.namespace, oldest.extmark_id)
    end
end

local function handle_lines(_event, bufnr, _changedtick, firstline, _lastline_old, lastline_new, _bytecount)
    ...
    for _, entry in ipairs(entries) do
        if entry.bufnr == bufnr then
            local region = entry_region(entry)
    ...
    if target then
        -- Grow the region with a fresh extmark; extmarks cannot be resized in
        -- place, but replacing the id keeps the entry.
        pcall(api.nvim_buf_del_extmark, bufnr, M.namespace, target.extmark_id)
        target.extmark_id = api.nvim_buf_set_extmark(bufnr, M.namespace, firstline, 0, {
        ...
        entries[#entries + 1] = {
            bufnr = bufnr,
            extmark_id = api.nvim_buf_set_extmark(bufnr, M.namespace, firstline, 0, {
                end_row = math.max(lastline_new, firstline + 1),
                right_gravity = false,
                end_right_gravity = true,
            }),
...
function M.setup(augroup)
    ...
    api.nvim_create_autocmd('BufWipeout', vim.tbl_extend('force', group, {
        callback = function(args)
            ...
            for i = #entries, 1, -1 do
                if entries[i].bufnr == args.buf then
                    table.remove(entries, i)
                end
            end
        end,
...
--- character budgets.
---@param bufnr integer the buffer the completion is for
---@return table[] chunks
function M.snapshot(bufnr)
    local options = require('harmonize').config.context_sources.recent_edits
    local chunks = {}
